...
@test "[VULNS] Deploy insecure image" {
    info
    deploy(){
        docker tag vulnerables/web-dvwa:1.9 harbor."${EXTERNAL_DNS}"/library/web-dvwa:1.9
        docker push harbor."${EXTERNAL_DNS}"/library/web-dvwa:1.9
    }
    run deploy
    [ "$status" -eq 0 ]
}
...
@test "[VULNS] Check insecure image is in the registry" {
    info
    test(){
        tag=$(curl -s -X GET "https://harbor.${EXTERNAL_DNS}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9/tags" \
...
@test "[VULNS] Scan an insecure image" {
    ...
    test(){
        ...
        curl -X POST "https://harbor.${EXTERNAL_DNS}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9/scan" \
            -H  "accept: application/json" \
        ...
        do
            scan_status=$(curl -s -X GET "https://harbor.${EXTERNAL_DNS}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9?with_scan_overview=true" \
                -H  "accept: application/json" \
        ...
        vulns=$(curl -s -X GET "https://harbor.${EXTERNAL_DNS}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9?with_scan_overview=true" \
            -H  "accept: application/json" \
