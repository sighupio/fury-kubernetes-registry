...
@test "[VULNS] Check insecure image is in the registry" {
    info
    test(){
        tag=$(curl -k -s -X GET "https://harbor.${TEST_DOMAIN}:${HTTPS_PORT}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9/tags" \
...
@test "[VULNS] Check scanner status" {
    info
    test(){
        health=$(curl -k -s -X GET "https://harbor.${TEST_DOMAIN}:${HTTPS_PORT}/api/v2.0/projects/1/scanner" \
            -H  "accept: application/json" \
...
@test "[VULNS] Scan an insecure image" {
    ...
    test(){
        ...
        curl -k -X POST "https://harbor.${TEST_DOMAIN}:${HTTPS_PORT}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9/scan" \
            -H  "accept: application/json" \
        ...
        do
            scan_status=$(curl -k -s -X GET "https://harbor.${TEST_DOMAIN}:${HTTPS_PORT}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9?with_scan_overview=true" \
                -H  "accept: application/json" \
        ...
        vulns=$(curl -k -s -X GET "https://harbor.${TEST_DOMAIN}:${HTTPS_PORT}/api/v2.0/projects/library/repositories/web-dvwa/artifacts/1.9?with_scan_overview=true" \
            -H  "accept: application/json" \
