...
@test "[SETUP] requirements - ingress" {
    info
    install_ingress(){
        cd katalog/tests/config
        furyctl vendor -H
        cd -
        kustomize build katalog/tests/config | kubectl apply -f -
    }
...
@test "[SETUP] requirements - Prepare Harbor manifests (externalIP)" {
    info
    files_to_change="""
    katalog/harbor/examples/full-harbor/kustomization.yaml
    katalog/harbor/examples/full-harbor/config/registry/config.yml
